 ...
 */
// TODO MR Delete
public class SMDeleteActionsLayout extends CustomComponent {
    ...
    // uploadViewConfirmationWindowLayout.getConsolidatedMessage();
    ...
    // event.getTransferable().getSourceComponent();
    // addToDeleteList(sourceTable, (TableTransferable)
    // event.getTransferable());
    // updateSWActionCount();
    // }
    // if
    // (sourceComponent.getId().startsWith(UIComponentIdProvider.UPLOAD_TYPE_BUTTON_PREFIX))
    // {
    //
    ...
    // .map(type ->
    // type.getName().equalsIgnoreCase(swModuleTypeName)).orElse(false)) {
    // notification.displayValidationError(
    // i18n.getMessage("message.swmodule.type.check.delete", new Object[] {
    // swModuleTypeName }));
    // } else {
    ...
    // }
    //
    // private void addToDeleteList(final Table sourceTable, final
    // TableTransferable transferable) {
    // final AbstractTable<?> swTable = (AbstractTable<?>) sourceTable;
    // final Set<Long> swModuleIdNameSet =
    // swTable.getSelectedEntitiesByTransferable(transferable);
    //
    // swModuleIdNameSet.forEach(id -> {
    // final String swModuleName = (String)
    // sourceTable.getContainerDataSource().getItem(id)
