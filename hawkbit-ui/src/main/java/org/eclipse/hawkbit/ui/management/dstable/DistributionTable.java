...
public class DistributionTable extends AbstractNamedVersionTable<DistributionSet, DistributionSetIdName> {
    ...
    void onDistributionSetUpdateEvents(final DistributionSetUpdatedEventHolder holder) {
        ...
        if (allOfThemAffectCompletedSetsThatAreNotVisible(holder.getEvents(), visibleItemIds)) {
            refreshDistributions();
        } else if (!checkAndHandleIfVisibleDsSwitchesFromCompleteToIncomplete(holder.getEvents(), visibleItemIds)) {
