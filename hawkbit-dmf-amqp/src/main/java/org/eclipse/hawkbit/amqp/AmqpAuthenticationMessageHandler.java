...
public class AmqpAuthenticationMessageHandler extends BaseAmqpService {
    ...
    private Optional<String> findArtifactByFileResource(final FileResource fileResource) {
        if (fileResource.getSha1() != null) {
            ...
            return artifactManagement.findArtifactByFilename(fileResource.getFilename()).map(org.eclipse.hawkbit.repository.model.Artifact::getSha1Hash);
        } else if (fileResource.getArtifactId() != null) {
            return artifactManagement.findArtifact(fileResource.getArtifactId()).map(org.eclipse.hawkbit.repository.model.Artifact::getSha1Hash);
