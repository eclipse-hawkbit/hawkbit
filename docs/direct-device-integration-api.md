...
The hawkBit [update server](management-api) provides REST resources which are consumed by the device to retrieve
...
> That allows users to have in some cases even multiple clients on the same target for different tasks, e.g. Firmware
...
the [Target State Machine](targetstate) of the hawkBit update server.
...
*DDI* also defines more states than the update server, e.g. multiple DDI states are currently mapped by the *DDI*
implementation to **RUNNING** state.  
...
See this [issue](https://github.com/eclipse-hawkbit/hawkbit/issues/952) for additional information, concerning the
