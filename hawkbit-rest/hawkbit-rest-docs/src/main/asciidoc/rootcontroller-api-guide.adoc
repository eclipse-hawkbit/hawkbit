...
Resource to receive information of the latest installation. Can be used to re-retrieve artifacts of the last finished action, for example in case a re-installation is necessary.
The response will be of the same format as the deploymentBase operation, providing the latest action that has been finished successfully.
