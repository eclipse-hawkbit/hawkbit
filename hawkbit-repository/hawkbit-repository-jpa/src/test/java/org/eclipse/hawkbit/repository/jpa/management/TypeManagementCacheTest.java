...
/**
 * Cache hit/miss behaviour (no ACM) for the DSType and SMType management services backed by the by-id cache.
 * <p>
 ...
 * <p>
 * DB access is counted via EclipseLink's {@code PerformanceMonitor} — the only reliable way to count SQL from the
 * ClientSessions that Spring transactions use ({@code SessionEventAdapter} on the ServerSession does not propagate
 * to ClientSessions and would always report 0).
 */
...
class TypeManagementCacheTest extends AbstractJpaIntegrationTest {
    ...
    void verifyRepeatedReadsOnlyMissHitsDb() {
        evictDstCache(standardDsType.getId());
    ...
    private void evictSmtCache(final Long id) {
        TenantAwareCacheManager.getInstance()
                .getCache(JpaSoftwareModuleType.class.getSimpleName())
                .evict(id);
    }

    /** Cumulative ReadAllQuery + ReadObjectQuery count from PerformanceMonitor. */
